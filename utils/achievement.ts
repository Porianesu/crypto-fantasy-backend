import { Achievement, Card, PrismaClient, User, UserAchievement, UserCard } from '@prisma/client'
...
export async function handleAchievementCardsCollect(user: User, cards: Array<Card>) {
  ...
  const tasks = [
    handleAchievementCardsCollectWithSubtype(user, 'amount', amount),
  ]
  ...
  if (legendary > 0) tasks.push(handleAchievementCardsCollectWithSubtype(user, 'legendary', legendary))
